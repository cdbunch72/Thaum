...
Posts a signed ``POST /{route_id}/log-level`` request to change the runtime
root log level on a running Thaum server. Credentials are read from an INI or
TOML profile (generated by :mod:`scripts.python.generate_admin_log_config`) or
supplied via CLI overrides.

Example::

    python scripts/python/thaum_log_override.py DEBUG --profile admin-client.ini
    python scripts/python/thaum_log_override.py default \\
        --post-url https://thaum.example.com/abc123/log-level \\
        --secret-b64url <key>
"""
...
def main() -> int:
    """Parse CLI arguments and POST a signed log-level change request.

    Returns:
        Process exit code (0 on success).

    Raises:
        SystemExit: When required credentials or URL components are missing,
