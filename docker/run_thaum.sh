...
if [ "$n" -ge "$MAX_RESTARTS" ]; then
  echo "[debug-131a48][H21] run_thaum: budget already exhausted (count=${n} max=${MAX_RESTARTS}) ppid=${PPID}"
  _signal_supervisord_if_budget_exceeded
  exit 1
fi
...
echo "[debug-131a48][H21] run_thaum start attempt=${n} max=${MAX_RESTARTS} ppid=${PPID}"
# endregion agent log
...
# region agent log
echo "[debug-131a48][H21] gunicorn exited rc=${rc} attempt=${n} max=${MAX_RESTARTS} ppid=${PPID}"
# endregion agent log
