...
- If **`db_url`** is empty:
  ...
  - Otherwise (unset/false): **bundled PostgreSQL** — the URL is built by `default_bundled_db_url()` as `postgresql+psycopg://…` over a **Unix socket** (`host` query parameter), using **peer** authentication (no password in the URL; the process OS user must match the role, e.g. `thaum` in the container). Optional: **`THAUM_PG_USER`** (default `thaum`), **`THAUM_PG_DATABASE`** (default `thaum`), **`THAUM_PG_SOCKET_DIR`** (default `/var/run/postgresql`).
