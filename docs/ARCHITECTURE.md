...
- **Container images** are built from one `Dockerfile` with build arg **`THAUM_BUNDLED_POSTGRES`** (install PostgreSQL + supervisord when `1`). CI publishes two name suffixes on the same registry base: default and **`-external-db`**. Cloud provider SDKs and optional secret backends (for example `gemstone_utils[azure]` for `azexp:`) are installed in **deploy-repo** images, not in upstream Thaum CI.
