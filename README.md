...
Each GitHub Release includes **`thaum-utils-<tag>.zip`**, its `.asc`, and binary **`SHA256SUMS.zip`**. The archive contains a `thaum-utils/` folder with `quickstart/`, `docs/`, `scripts/`, `sample.thaum.toml`, and `incident_prompt_card.sample.j2`. Source integrity is committed **`SHA256SUMS.txt`** / **`SHA256SUMS.txt.asc`** (text-mode GNU `sha256sum -c`; `#` header names version, date, **hashed-commit**, and coverage).
