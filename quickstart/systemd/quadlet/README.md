...
- A Thaum container image:
  - **Published (CI)**: pull from GitHub Container Registry, e.g. `podman pull ghcr.io/cdbunch72/thaum:latest` (default in [`thaum.container`](thaum.container)). Other tags (`:devel`, `:edge` / `:edge-<branch>`, `:<version>`) are described in [README.md](../../../README.md) “Container images (CI)”.
