...
- A Thaum container image:
  - **Published (signed, GHCR `thaum`)**: pull from GitHub Container Registry, e.g. `podman pull ghcr.io/gemstone-software-dev/thaum:latest` (default in [`thaum.container`](thaum.container)). Version / `:devel` / `:edge` tags are described in [README.md](../../../README.md) “Container images”. Unsigned `thaum-debug` images are CI-only smoke builds, not for production.
